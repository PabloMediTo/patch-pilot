...
const envelope = { deliveryId: "delivery-1", eventName: "issues",
  observedAt: "2026-08-16T14:00:00.000Z", payload: { action: "labeled",
    ...
    issue: { number: 42, title: "Correct the reported assertion",
      body: "The calculation returns an incorrect result.\n\n<!-- patch-pilot:expected-failure -->\nreported assertion\n<!-- /patch-pilot:expected-failure -->" },
    sender: { id: 23 } } };
...
assert.equal(savedRuns[0].issueTitle, "Correct the reported assertion");
assert.equal(savedRuns[0].issueContext, "The calculation returns an incorrect result.");
