...
- Canonical MVP command and change policy is enforced before a sandbox port. A Docker adapter selects pinned runtimes, applies the fixed limits, copies the workspace into the quota-controlled container layer, and guarantees cleanup. The worker now supplies bounded shell-free Docker process execution; application composition and live Docker proof remain required before target-repository commands can run.
