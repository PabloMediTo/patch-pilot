...
- The web shell renders a safe review document, exposes an authenticated GET handler, loads a same-origin EventSource asset, and dispatches local review resources while forwarding only timeline and approval route shapes; concrete listener/session/API transport wiring and browser-level visual verification remain open.
