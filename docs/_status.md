...
- The web shell renders a safe review document, exposes an authenticated GET handler, loads a same-origin EventSource asset, and uses a concrete Node server to dispatch local resources while streaming only timeline and approval route shapes to the API; main-process session/evidence composition and browser-level visual verification remain open.
