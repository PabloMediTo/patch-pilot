...
- Canonical MVP command and change policy is enforced before a sandbox port. A Docker adapter now selects pinned runtimes, applies the fixed limits, copies the workspace into the quota-controlled container layer, and guarantees cleanup; a worker process port and live Docker proof remain required before target-repository commands can run.
