...
- The worker has an executable Temporal deployment and bundled maintenance workflow that records replay-safe inspection, reproduction, planning-context, and proposal events, uses fresh disposable exact-revision checkouts for all three repository Activities, invokes the safe executor for supported projects, generates strict structured plan/diff candidates through a worker-owned OpenAI adapter, independently validates proposals in the maintenance package, terminates blocked outcomes explicitly, and closes Temporal, Postgres, and Redis resources deterministically. Live OpenAI and Docker proof remain open.
