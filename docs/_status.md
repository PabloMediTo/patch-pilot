...
- The worker has an executable Temporal deployment and bundled maintenance workflow that records replay-safe inspection, reproduction, and planning-context events, uses fresh disposable exact-revision checkouts for all three repository Activities, invokes the safe executor for supported projects, gates planning on accepted reproduction plus bounded context, terminates every other known outcome explicitly, and closes Temporal, Postgres, and Redis resources deterministically. Live Docker proof remains open.
