...
- The web shell renders a safe, responsive review document, exposes an authenticated GET handler, loads same-origin style and EventSource assets, and uses a concrete Node server to dispatch local resources while streaming only timeline and approval route shapes to the API. Desktop, 375-pixel mobile, overflow, controls, and live insertion are browser-verified; main-process session/evidence composition remains open.
