...
- The maintenance package index composes all public maintenance concepts. Explicit package-module edges are limited to `change-proposals` → `safety`, `repository-understanding` → `safety`, and `proposal-attempts` → `verifications`/`critiques`; `approvals` remains independent. Worker application-level edges are `sandbox-execution` → `docker-cli`, `maintenance-workflow` → `sandbox-execution`, and `application` → `maintenance-workflow`/`proposal-generation`. Proposal materialization adds exact `node:buffer` access to `repository-workspaces`; no module edge, external dependency, or technical exception is introduced.
