...
- The worker exposes `github-delivery` as the credentialed Activity role that loads canonical approval evidence and composes the maintenance package's delivery ports. It receives only the maintenance workspace edge already available to the worker and the exact `pg` provider permission for its owned pool; publication lifecycle is separate from deterministic workflow code.
- The worker exposes `repository-access` as the read-only checkout-credential role. It composes the maintenance package's GitHub authentication interface with `contents:read`, maps the opaque token to GitHub's HTTP Basic Git credential through exact `node:buffer`, and returns only a bounded authorization header.
...
- The worker exposes `maintenance-workflow` as the application role that owns deterministic orchestration and concrete Activities with only the exact `@temporalio/workflow` provider. Its Activities depend on the public `sandbox-execution` role to reproduce failures without weakening policy and receive repository authorization through an injected port. The separate `proposal-generation` application role owns OpenAI Responses HTTP composition and receives only `node:buffer` for response-size enforcement. The worker `application` role alone imports `@temporalio/worker` and `node:url`, composes workflow, proposal-generation, repository-access, and GitHub-delivery public interfaces, and `main.js` receives only `node:process`.
...
- The maintenance package also exposes `repository-workspaces`, which owns disposable Git checkout, exact-base proposal materialization, and cleanup and uses only its exact buffer, filesystem, process, path, and utility core providers.
- The maintenance package exposes `github-authentication`, which owns repository-scoped App JWT exchange, exact permission profiles, refresh coalescing, and memory-only installation-token caches with exact buffer, cryptography, timer, and URL core providers. `deliveries` depends on only its public interface; read-only worker repository access composes it through the package root.
...
- The API `application` role owns deployment composition and imports the `github-delivery`, `github-ingestion`, `workflow-submission`, and `workflow-approval` public interfaces plus exact `pg` and `@temporalio/client`; it creates one shared pool for all API Postgres stores and one shared Temporal client resource for workflow commands. Its `github-delivery` role is limited to persisted webhook reconciliation; approved publication is worker-owned. API `main.js` uses only `application` and exact `node:process` for listener startup and shutdown signals.
- The API shell exposes `workflow-submission` as the application role that starts persisted runs and imports `@temporalio/client` only for its exact already-started error class. The separate provider-free `workflow-approval` role validates and signals only canonical bound decisions through an injected Temporal client. No direct edge connects these roles; `application` composes both over its owned client resource.
...
- The maintenance package index composes all public maintenance concepts. Explicit package-module edges are `change-proposals` → `safety`, `repository-understanding` → `safety`, `proposal-attempts` → `verifications`/`critiques`, and `deliveries` → `github-authentication`; `approvals` remains independent. Worker application-level edges are `sandbox-execution` → `docker-cli`, `maintenance-workflow` → `sandbox-execution`, and `application` → `github-delivery`/`maintenance-workflow`/`proposal-generation`/`repository-access`. The worker delivery role receives exact `pg`; no technical exception is introduced.
