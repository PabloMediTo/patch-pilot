...
- No Temporal workflow, authenticated GitHub App request transport, safe remote commit publisher, control-plane API main-process authentication/store composition, or dependency restoration has been implemented.
...
Implement short-lived GitHub App request authentication and safe approved-commit publication behind the GitHub REST adapter, then compose the complete delivery path. Control-plane API authentication/store composition and live Postgres/Redis and Docker safety verification remain open where local runtime or policy decisions are still required.
