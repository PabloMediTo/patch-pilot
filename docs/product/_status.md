...
- No Temporal workflow, complete control-plane API main-process composition, or dependency restoration has been implemented. The API application runtime constructs the environment-configured single-operator bearer adapter, forces review, timeline, and approval routes to share its authentication and all-runs authorization ports, and composes review-snapshot, timeline, and approval stores into the read and decision queries. Environment-backed store creation, listener startup, and service lifecycle wiring remain open.
...
Wire the executable API main-process listener and lifecycle around the implemented authentication, review query, persisted stores, and live stream. Live Postgres/GitHub, Postgres/Redis, and Docker safety verification remain open where local runtime availability is still required.
