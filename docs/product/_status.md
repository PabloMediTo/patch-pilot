...
- No Temporal workflow, control-plane API main-process authentication/store composition, complete GitHub-delivery runtime composition, or dependency restoration has been implemented.
...
- A provider-free [GitHub delivery](../DICTIONARY.md#github-delivery) use case now gates publication against the exact approval binding, hashes the current diff, derives one safe deterministic branch, requires a linked draft pull request, and normalizes durable or concurrent retries. Its Postgres store atomically persists complete evidence. A concrete GitHub REST adapter creates or exactly replays branch refs and open draft PRs, recovers concurrent `422` responses, and refuses changed provider state. Its commit publisher rechecks the diff hash, strictly applies bounded UTF-8 text patches to the exact base tree, and creates deterministic Git trees and child commits; runtime composition and live verification remain open.
...
Compose the authenticated request transport, deterministic commit publisher, GitHub delivery adapter, approval/store ports, and delivery use case into one complete runtime path. Control-plane API authentication/store composition and live Postgres/GitHub, Postgres/Redis, and Docker safety verification remain open where local runtime or policy decisions are still required.
