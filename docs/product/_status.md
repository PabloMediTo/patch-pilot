...
- A [run timeline](../DICTIONARY.md#run-timeline) module now provides an idempotent Postgres schema, atomic per-run event sequences, ordered history queries, and run-scoped Redis publication/subscription. The API composes these into a gap-free, deduplicated history-to-live feed and resumable SSE session. Its adapters are unit-tested but still require live verification against the local services.
- No Temporal workflow, concrete GitHub API adapter, HTTP server/router, visual frontend, dependency restoration, or container command sandbox has been implemented.
