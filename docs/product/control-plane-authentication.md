...
The application runtime constructs this adapter from deployment values and gives the concrete server one shared authentication source. Individual review, timeline, or approval integrations cannot replace it with route-specific authentication ports. The executable [control-plane runtime](control-plane-runtime.md) now composes that policy with the persisted stores, Redis stream, and listener lifecycle.
