...
The command invokes Docker with exact argument vectors and no shell. It checks the Docker engine, Docker Compose, and `docker compose config --quiet`. It also validates the presence of provider/runtime configuration, the minimum bearer-token length, a positive GitHub App identifier, two distinct case-insensitively compared `owner/repository` pilot targets, and positive issue numbers no greater than the durable Postgres `integer` maximum of `2147483647`. Reusing one repository for both language roles marks both repository variable names invalid without exposing either value. Oversized issue identities likewise report only the invalid variable name.
