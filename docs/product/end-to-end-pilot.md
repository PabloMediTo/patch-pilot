...
The command invokes only Docker with exact argument vectors, no shell, a ten-second timeout, a 64-KiB output limit, and forced termination after timeout. It checks the Docker engine, Docker Compose, and `docker compose config --quiet`. It also validates the presence of provider/runtime configuration, the API runtime's exact whitespace-free minimum bearer-token shape, its safe 1-to-128-character actor identity, a positive GitHub App identifier, locally parseable private-key material, two distinct case-insensitively compared `owner/repository` pilot targets, and positive issue numbers no greater than the durable Postgres `integer` maximum of `2147483647`. Optional API, web, Temporal, storage, workspace, and OpenAI-model overrides may remain absent to select deployment defaults; when explicitly set, they must be non-empty, listener ports must be within `1..65535`, and the web API origin must use HTTP or HTTPS. Reusing one repository for both language roles marks both repository variable names invalid without exposing either value. Malformed authentication, key, or override values and oversized issue identities likewise report only invalid variable names. Parsing the key proves only that both deployments can start with it; GitHub still has to authenticate it during live execution.
...
4. Run `npm run test:sandbox-integration` to retain real cgroup, filesystem, network, privilege, timeout, output-bound, and cleanup proof.
