...
The commit-publication port still owns applying the approved diff and making the resulting commit available in the target Git database. Keeping workspace mutation outside authentication and REST replay logic prevents credentials, patch application, and PR identity handling from becoming one inseparable mechanism.
...
The provider-free use case, Postgres store, authenticated GitHub App request transport, GitHub reference/draft-PR adapter, and focused tests are implemented. Live Postgres/GitHub verification and safe commit publication remain planned.
