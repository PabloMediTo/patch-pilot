...
- an accepted initial run bound to the GitHub delivery, installation, repository, default branch, issue, actor, resolved immutable base revision, and exact expected-failure fragment
...
The opted-in issue body must contain exactly one bounded expected-failure marker pair. The trimmed text between the markers is persisted verbatim and later matched as an exact output fragment during reproduction. Missing, empty, duplicated, or oversized marker content is rejected as a malformed triggering payload instead of inferring failure evidence from unrestricted issue prose.
