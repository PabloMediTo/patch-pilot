...
Only a validated `reproduced` outcome starts [repository planning context](../DICTIONARY.md#repository-planning-context) collection in a third fresh exact-revision checkout. Ready context records `run.planning.ready` with paths and byte metrics while retaining full text only in the Activity result. A separate generator Activity then calls the worker-owned [proposal generator](../DICTIONARY.md#proposal-generator), passes its structured candidates through the provider-free change-proposal boundary, and records proposal lifecycle events without publishing the source diff. Safe proposals enter a 30-minute [proposal-attempt](../DICTIONARY.md#proposal-attempt) Activity in a fourth fresh checkout. Each attempt restores the immutable base, applies the complete diff, verifies through the safe executor, critiques passed evidence, and permits at most two full proposal revisions. Accepted attempts become `attempts-completed`; rejected, exhausted, and policy-blocked outcomes terminate explicitly. Unknown Activity classifications record a phase-specific failure and fail the workflow instead of becoming product evidence. The workflow does not yet persist the final review snapshot, wait for approval, or deliver a pull request, and live Docker enforcement still requires runtime proof.
