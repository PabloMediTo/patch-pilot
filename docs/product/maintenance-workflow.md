...
- GitHub delivery uses at most five 30-minute Activity attempts without rerunning human approval.
...
The workflow passes the exact reviewed source diff, base revision, plan version, and verification identity to the delivery Activity. The Activity independently reloads the canonical approval decision from Postgres before the provider-free gate runs. Timeline events retain branch, immutable head, draft-pull-request identity, and delivery classification while excluding the source diff and idempotency key.
