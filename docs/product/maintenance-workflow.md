...
Only a validated `reproduced` outcome starts [repository planning context](../DICTIONARY.md#repository-planning-context) collection in a third fresh exact-revision checkout. Ready context records `run.planning.ready` with paths and byte metrics while retaining full text only in the Activity result. A separate generator Activity then calls the worker-owned [proposal generator](../DICTIONARY.md#proposal-generator), passes its structured candidates through the provider-free change-proposal boundary, and records proposal lifecycle events without publishing the source diff. Safe proposals end this phase as `proposal-ready`; policy-blocked proposals terminate explicitly. Unsupported, not-reproduced, different-failure, execution-failed, unavailable-context, and blocked-proposal outcomes record `run.terminal` and cannot continue. Unknown Activity classifications record a phase-specific failure and fail the workflow instead of becoming product evidence. The workflow does not yet apply and verify proposal attempts, persist the final review snapshot, wait for approval, or deliver a pull request, and live Docker enforcement still requires runtime proof.
