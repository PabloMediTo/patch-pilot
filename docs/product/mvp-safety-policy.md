...
After the canonical sandbox probe succeeds, the same command exercises the real bounded Docker CLI process port with two short controlled containers. One intentionally exceeds a 64-KiB test output bound and the other exceeds a one-second test timeout. The verifier requires the matching truncation or timeout classification, forcibly removes both containers, and confirms that neither remains. These deliberately shorter probe bounds test the process port without changing the canonical 10-minute and 1-MiB production policy. The JSON report contains only fixed check names and statuses.
