...
The control-plane deployment now has a concrete Node listener and one dispatcher for review evidence, approval commands, and timeline SSE. Its transport owns bounded JSON or form parsing, SSE heartbeat scheduling, stable 404 behavior, and safe error termination. Authentication, persistence stores, and timeline providers remain injected role ports until the API main process composes real deployment configuration.

The maintenance package now owns provider-free [GitHub delivery](github-delivery.md) behavior as a separate conceptual module. It depends only on hashing for stable diff and branch identities; GitHub App calls and atomic delivery persistence remain injected ports so provider mechanics do not enter the approval gate.
