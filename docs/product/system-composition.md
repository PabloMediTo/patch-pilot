...
The control-plane deployment now has a concrete Node listener and one dispatcher for review evidence, approval commands, and timeline SSE. Its transport owns bounded JSON or form parsing, SSE heartbeat scheduling, stable 404 behavior, and safe error termination. Its application runtime creates the [control-plane authentication](control-plane-authentication.md) role from environment values and supplies the same immutable bearer operator ports to approval, review-evidence, and timeline routes; route integrations cannot substitute competing authentication policies. Its `github-delivery` application role composes one managed Postgres pool, the approval and delivery stores, GitHub App transport, deterministic commit publisher, branch/PR adapter, and delivery use case behind one operation with idempotent shutdown. General query providers and environment-backed pool, Redis, listener, and lifecycle composition remain unwired in the executable API main process.
