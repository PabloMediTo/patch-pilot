...
The executable worker registers the workflow and advances a submitted run through inspection, reproduction, planning context, bounded proposal generation, proposal attempts, canonical review-snapshot recording, an exactly bound durable approval wait, and idempotent approved delivery. The API's separate workflow-approval role wakes that wait; rejection remains terminal without scheduling the delivery Activity.
