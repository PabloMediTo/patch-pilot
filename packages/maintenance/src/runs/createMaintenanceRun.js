 ...
 *
 * @param {{ id: string, installationId?: number, repository: string, issueNumber: number, issueTitle: string, issueContext: string, expectedFailure: string, defaultBranch?: string, baseRevision: string, actorId?: number, sourceDeliveryId?: string }} input Run identity, authorization context, and immutable target.
 * @returns {{ id: string, installationId?: number, repository: string, issueNumber: number, issueTitle: string, issueContext: string, expectedFailure: string, defaultBranch?: string, baseRevision: string, actorId?: number, sourceDeliveryId?: string, status: string }} The initial run state.
 */
...
function assertRunInput(input) {
  const hasStrings = [input?.id, input?.repository, input?.baseRevision, input?.issueTitle,
    input?.issueContext, input?.expectedFailure]
  ...
  const hasBoundedIssue = input?.issueTitle?.length <= 500
    && input?.issueContext?.length <= 8000 && input?.expectedFailure?.length <= 500;
  ...
  if (!hasStrings || !hasRepository || !hasRevision || !hasIssue || !hasBoundedIssue
